# Session Notes MOC

Use this MOC to navigate session records, recaps, and raw notes awaiting cleanup.

## Parsed Session Notes

- [[Daylight and Dragons/parsed_notes/2026-06-27|2026-06-27 Game Log]]
- [[Daylight and Dragons/parsed_notes/2026-08-22|2026-08-22 Game Log]]

## Raw Session Notes

- `raw_notes/20260627-game-log.md` - parsed to [[Daylight and Dragons/parsed_notes/2026-06-27|2026-06-27 Game Log]]
- `raw_notes/20260822-session-2-notes.md` - parsed as Session 3 to [[Daylight and Dragons/parsed_notes/2026-08-22|2026-08-22 Game Log]]

## Session Index

### Session 1

- Source: `raw_notes/20260627-game-log.md`
- Parsed note: [[Daylight and Dragons/parsed_notes/2026-06-27|2026-06-27 Game Log]]
- Status: Parsed recap only
- Key locations: [[The Mound]], [[Dragoncrest]]
- Key NPCs: [[Nelly]]
- Key discoveries: masked attackers were [[dragonborn]]

### Session 2

- Source: `raw_notes/20260627-game-log.md`
- Parsed note: [[Daylight and Dragons/parsed_notes/2026-06-27|2026-06-27 Game Log]]
- Status: Parsed
- Key locations: [[Dragoncrest]], [[The Cast Iron]]
- Key participants: [[Rowan I]], [[Mark]], [[Lio]], [[Mattalic Longlisten]]
- Key discoveries: [[The Cast Iron]] appears connected to the masked attackers; a magic coin bag emptied itself as the party moved away.

### Session 3

- Source: `raw_notes/20260822-session-2-notes.md`
- Parsed note: [[Daylight and Dragons/parsed_notes/2026-08-22|2026-08-22 Game Log]]
- Status: Parsed
- Key locations: [[The Mound]], [[Undercity]], [[Nelly's Home]], [[Lio's House]]
- Key participants: [[Rowan I]], [[Mark]], [[Lio]], [[Mattalic Longlisten]], [[Nelly]]
- Key discoveries: [[Nelly]] owes [[Drunken Soberbeard]], [[Nelly's Son]] is missing, and [[Cast Iron Elf]] disappeared with [[Mark]].
- Note: The raw note filename still says `session-2` because it was the user's second attended session; the campaign group numbering is Session 3.

## Session Note Template

````markdown
# Session YYYY-MM-DD

## Recap

Brief summary of what mattered from the previous session.

## Cast

- [[Player Character]]
- [[NPC]]

## Scenes

### S1 *Scene name or location*

```
@(Character) Action
d: Roll or resolution -> Result
=> Consequence
```

## Discoveries

- 

## Threads Updated

- 

## Open Questions

- 
````

## Maintenance

- After parsing a raw note, add the parsed note under `Parsed Session Notes`.
- Update [[Daylight and Dragons/MOCs/Active Threads MOC|Active Threads MOC]] with new hooks or resolved plots.
- Update [[Daylight and Dragons/MOCs/Timeline MOC|Timeline MOC]] with major events.
- Update [[Daylight and Dragons/MOCs/Open Questions MOC|Open Questions MOC]] with unresolved mysteries.
